...
function printFile(fileToRead){
    ...
    //AHHHHHHHHHHHHHHHHHHHH - work on here
    //
}
...
function replaceCamelWithSpace(string){
    var splitted = string.split('');
    var withSpace = "";

    splitted.forEach((char, i) => {
        var isUppercase = /[A-Z]/.test(char); //regex test if capital
        if(isUppercase && i != 0){ //if capital and not the first char
            withSpace += " ";
        }
        withSpace += char;
    })
